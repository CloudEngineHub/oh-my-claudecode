...
export function parseXargsOption(value: string): XargsOptionResult {
  ...
  if (value === "--") return 1;
  if (/^(?:-d|--delimiter|-E|--eof|-I|--replace|-L|--max-lines|-n|--max-args|-P|--max-procs|-s|--max-chars|--process-slot-var)$/.test(value)) {
    return 2;
  }
  if (/^(?:--delimiter=|--eof=|--replace=|--max-lines=|--max-args=|--max-procs=|--max-chars=|--process-slot-var=)/.test(value)) {
    return 1;
  }
